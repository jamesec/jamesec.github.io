...
Although it's possible to save the webpage with ChatGPT as an .mhtml file by using "Save Page As..." in Chromium/Chrome and selecting "Webpage, Single File" as the format. However, the resulting .mhtml file is usually much larger than the file generated by the `SingleFile` extension.

Additionally, the .mhtml file embeds images as base64 data within the file. In contrast, when you configure `SingleFile` like this: Options → File format → self-extracting ZIP (universal), the resulting ZIP package from `SingleFile` can be extracted, and the images are preserved in their original format.
